...
- [ ] 1.1 ERPNext-Service-Account (API-Key/Secret) anlegen und Rechte prüfen — Anleitung in README (deine ERPNext-Instanz)
...
- [ ] 1.6 `mcpo`-Proxy vor OpenWebUI deployen und Tool-Server in OpenWebUI registrieren — Config `deploy/mcpo.config.json` + Anleitung geliefert; Deploy auf deiner VM
- [ ] 1.7 End-to-end-Nachweis: `finde_kunde("...")` über OpenWebUI → mcpo → MCP → ERPNext grün — braucht Live-Instanz
- [ ] 1.8 Direkter MCP-Zugriff (Claude/Claude Code) gegen denselben Server verifizieren — braucht Live-Instanz

## 2. Phase 1 — Vertriebs-Rücken

- [ ] 2.1 `kontakt_anlegen` (Lead anlegen; Lead→Customer konvertieren)
- [ ] 2.2 `angebot_erstellen`: Party Lead/Customer, Hybrid-Item (finde_artikel + generischer Fallback), Preis aus Preisliste mit Override, 19%-Steuer-Default; Draft anlegen, Submit unter Guardrail
- [ ] 2.3 `angebot_annehmen` via `make_sales_order` → Sales-Order-ID
- [ ] 2.4 `projekt_anlegen` aus Sales Order (Project + optionale Tasks)
- [ ] 2.5 `finde_projekt` für die Projekt-Auflösung ergänzen (Vorbedingung für 2.7)
- [ ] 2.6 `zeit_erfassen`: an offenes Draft-TS je (Projekt, Mitarbeiter) anhängen; `billing_hours`-Override; Warnung bei Satz = 0
- [ ] 2.7 `rechnung_aus_zeiten`: alle offenen abrechenbaren TS eines Projekts sammeln, submitten und zu EINER Sales Invoice aggregieren (`timesheets[]` selbst befüllen, Zeile je Tätigkeitsart)
- [ ] 2.8 Submit/Cancel-Bestätigungs-Guardrail implementieren (Entwurf frei, Verbuchen nur nach Bestätigung; zwei Submit-Punkte in 2.7)
- [ ] 2.9 `zahlung_verbuchen` (Payment Entry zu Invoice, Submit gemäß Guardrail)
- [ ] 2.10 Gesamten Flow Kontakt→Angebot→Annahme→Projekt→Zeit→Rechnung end-to-end testen
