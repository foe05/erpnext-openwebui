...
Der Vertriebs-Rücken: der komplette ERPNext-Zyklus per Chat — Kontakt anlegen, Angebot erstellen und annehmen, Projekt anlegen, Zeit erfassen, aus Zeiten abrechnen und Zahlung verbuchen. Übergänge nutzen ERPNext-eigene Methoden; verbuchende Schritte (submit/cancel) stehen unter einem Bestätigungs-Guardrail.
...
Der Connector SHALL die Annahme einer Quotation über die ERPNext-Übergangsmethode `make_sales_order` in einen Sales Order überführen. Ein Angebot an einen Lead erfordert vorher die Lead→Kunde-Konvertierung. Pflichtfelder des Auftrags (z. B. Lieferdatum) werden dabei gesetzt.

#### Scenario: Annahme erzeugt Auftrag
- **WHEN** `angebot_annehmen(QTN-0042)` mit einer echten Quotation-ID an einen Kunden bestätigt aufgerufen wird
- **THEN** submittet der Connector das Angebot, ruft `make_sales_order` auf, setzt das Lieferdatum und gibt die echte Sales-Order-ID zurück

#### Scenario: Annahme eines Lead-Angebots
- **WHEN** das anzunehmende Angebot an einen Lead gerichtet ist
- **THEN** weist der Connector darauf hin, dass zuerst `lead_zu_kunde` nötig ist, statt einen Auftrag ohne Kunden zu erzeugen

### Requirement: Projekt aus Auftrag anlegen
Der Connector SHALL zu einem Sales Order ein Project anlegen, das mit Auftrag/Kunde verknüpft ist und für die Zeiterfassung verwendet werden kann.
...
Der Connector SHALL abrechenbare Zeiten als Zeilen an ein offenes Draft-Timesheet anhängen, das je Kombination aus Projekt und Mitarbeiter genau einmal geführt und erst durch die Abrechnung geschlossen wird. Der Abrechnungssatz wird aus der Activity Cost ermittelt; Pflicht-Zeitfenster (from/to) werden überlappungsfrei gesetzt.
...
- **THEN** liefert der Connector zunächst eine Vorschau zurück und führt das Submit erst nach ausdrücklicher Bestätigung (bestaetigen=true) aus; ohne Bestätigung bleibt das Dokument im Entwurf
...
- **THEN** legt der Connector einen Payment Entry an, der der Rechnung zugeordnet ist, und behandelt das Submit gemäß der Bestätigungsregel
